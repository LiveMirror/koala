...
public class RoleApplicationImpl extends BaseImpl implements RoleApplication {
		...
		return queryChannel()
				.createJpqlQuery(
						"select new org.openkoala.auth.application.vo.UserVO(user.id,user.name,"
								...
								+ "from RoleUserAuthorization rua join rua.user user join rua.role role where role.id=:roleId and rua.abolishDate>:abolishDate")
				.addParameter("roleId", roleVO.getId()).addParameter("abolishDate", new Date()).list();
	}
	...
	public Page<RoleVO> pageQueryRole(int currentPage, int pageSize) {
		List<RoleVO> results = new ArrayList<RoleVO>();
		Page<Role> pages = queryChannel().createJpqlQuery("select m from Role m where m.abolishDate>:abolishDate").addParameter("abolishDate", new Date())
	...
	public Page<RoleVO> pageQueryRoleByUseraccount(int currentPage, int pageSize, String useraccount) {
		...
		Page<Role> pages = queryChannel()
				.createJpqlQuery(
						"select role from RoleUserAuthorization rau " + "join rau.role role join rau.user user where user.userAccount=:useraccount "
								+ "and rau.abolishDate>:abolishDate").addParameter("useraccount", useraccount).addParameter("abolishDate", new Date())
				.setPage(currentPage, pageSize).pagedList();
		for (Role each : pages.getData()) {
			RoleVO roleVO = new RoleVO();
			roleVO.domain2Vo(each);
	...
	public Page<RoleVO> pageQueryByRoleCustom(int currentPage, int pageSize, QueryConditionVO query) {
		...
		Page<Role> pages = queryChannel().createJpqlQuery(genQueryCondition(query)).addParameter("abolishDate", new Date()).setPage(currentPage, pageSize)
				.pagedList();
		...
		return new Page<RoleVO>(pages.getStart(), pages.getResultCount(), pages.getPageSize(), result);
	}

	public Page<RoleVO> pageQueryNotAssignRoleByUseraccount(int currentPage, int pageSize, String useraccount, RoleVO roleVO) {
		...
		Page<RoleVO> pages = queryChannel().createJpqlQuery(jpql.toString()).setParameters(conditions).setPage(currentPage, pageSize).pagedList();
	...
	public Page<UserVO> pageQueryNotAssignUserByRole(int currentPage, int pageSize, UserVO userVO, RoleVO roleVO) {
		...
		StringBuilder jpql = new StringBuilder("select user from User user where user not in(" + "select user from RoleUserAuthorization auth" //
		...
		return new Page<UserVO>(pages.getStart(), pages.getResultCount(), pages.getPageSize(), result);
	}
}
