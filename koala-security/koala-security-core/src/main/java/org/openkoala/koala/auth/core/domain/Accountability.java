...
public abstract class Accountability extends TimeIntervalEntity {
	 ...
	 * @return
	 */
	public static <T extends Accountability> T get(Class<T> entityClass,
			Party commissioner, Party responsible, Date queryDate) {
		return getRepository().createCriteriaQuery(entityClass)
				.eq(COMMISSIONER, commissioner).eq(RESPONSIBLE, responsible)
				.le(CREATE_DATE, queryDate).gt(ABOLISH_DATE, queryDate)
				.singleResult();
	}
	 ...
	 * @return
	 */
	public static <T extends Accountability> List<T> findAccountabilities(
			Class<T> entityClass, Date queryDate) {
		return getRepository().createCriteriaQuery(entityClass)
				.le(CREATE_DATE, queryDate).gt(ABOLISH_DATE, queryDate)
				.list();
	}
	 ...
	 * @return
	 */
	public static <T extends Accountability> List<T> findByCommissioner(
			Class<T> entityClass, Party commissioner) {
		return getRepository().createCriteriaQuery(entityClass).eq(COMMISSIONER, commissioner).asc(CREATE_DATE).list();
	}
	 ...
	 */
	public static <T extends Accountability> List<T> findByCommissioner(
			Class<T> entityClass, Party commissioner, Date queryDate) {
		return getRepository().createCriteriaQuery(entityClass).eq(COMMISSIONER, commissioner)
		.le(CREATE_DATE, queryDate).gt(ABOLISH_DATE, queryDate).list();
	}
	 ...
	 * @return
	 */
	public static <T extends Accountability> List<T> findByCommissioners(
			Class<T> entityClass, Collection<? extends Party> commissioners,
			Date queryDate) {
		if (commissioners == null || commissioners.isEmpty()) {
			return new ArrayList<T>();
		}
		return getRepository().createCriteriaQuery(entityClass).in(COMMISSIONER, commissioners)
				.le(CREATE_DATE, queryDate).gt(ABOLISH_DATE, queryDate).list();
	}
	 ...
	 * @return
	 */
	public static <T extends Accountability> List<T> findByResponsible(
			Class<T> entityClass, Party responsible) {
		return getRepository().createCriteriaQuery(entityClass).eq(RESPONSIBLE, responsible)
				.asc(CREATE_DATE).list();
	}
	 ...
	 */
	public static <T extends Accountability> List<T> findByResponsible(
			Class<T> entityClass, Party responsible, Date queryDate) {
		return getRepository().createCriteriaQuery(entityClass).eq(RESPONSIBLE, responsible)
				.le(CREATE_DATE, queryDate).gt(ABOLISH_DATE, queryDate).list();
	 ...
	 * @return
	 */
	public static <T extends Accountability> List<T> findByResponsibles(
			Class<T> entityClass, Collection<? extends Party> responsibles,
			Date queryDate) {
		if (responsibles == null || responsibles.isEmpty()) {
			return new ArrayList<T>();
		}
		return getRepository().createCriteriaQuery(entityClass).in(RESPONSIBLE, responsibles)
				.le(CREATE_DATE, queryDate).gt(ABOLISH_DATE, queryDate).list();
	}
	...
	public static Set<Accountability> findByParty(Party party, Date queryDate) {
		...
		results.addAll(findByCommissioner(Accountability.class, party,
				queryDate));
		return results;
	}
	...
	public boolean equals(Object other) {
		...
		return new EqualsBuilder().append(this.getCreateDate(),
				that.getCreateDate()).isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(getCreateDate()).toHashCode();
	}

	@Override
	public String toString() {
		return "createDate: " + getCreateDate() + ", abolishDate: "
				+ getAbolishDate();
	}
}